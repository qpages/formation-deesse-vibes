...
vi.mock('../../../lib/enrollment/reconcile', () => ({ reconcileEnrollment }));

import { POST } from './nda-sync';
import { RATE_LIMITS, enforceRateLimit, rateLimitKey, resetRateLimitStoreForTests } from '../../../lib/rate-limit';

function postNdaSync(cookie?: string) {
	return POST({
		request: new Request('http://localhost/api/enrollment/nda-sync', {
			method: 'POST',
...
describe('POST /api/enrollment/nda-sync', () => {
	it('sans cookie → 401', async () => {
		parseCookie.mockReturnValue(null);

		const res = await postNdaSync();
	...
	it('session invalide → 401', async () => {
		...
		const res = await postNdaSync('bad-token');
		expect(res.status).toBe(401);
	...
	it('signature confirmée → signed true', async () => {
		...
		const res = await postNdaSync('valid');
	...
	it('pas en attente → 409 not_awaiting', async () => {
		parseCookie.mockReturnValue('valid');
		verifyEnrollmentSessionToken.mockResolvedValue('enr_1');
		reconcileEnrollment.mockResolvedValue({
			enrollmentId: 'enr_1',
			trigger: 'client.nda_sync',
			scope: 'nda_signature',
			steps: [{ step: 'nda_signature', status: 'failed', reason: 'not_awaiting' }],
			mutated: false,
		});
