...
vi.mock('./env', () => ({
	getEnv: () => ({ YOUSIGN_API_BASE: 'https://api-sandbox.yousign.app/v3' }),
...
import { downloadSignedDocuments, ndaSignatureRequestName } from './yousign';
...
describe('downloadSignedDocuments', () => {
	beforeEach(() => {
	...
	it('GET version=completed en binaire', async () => {
		...
		const file = await downloadSignedDocuments('req-uuid');
	...
	it('erreur HTTP → throw avec le body', async () => {
		...
		await expect(downloadSignedDocuments('req-uuid')).rejects.toThrow('Yousign 400: not done');
	});
});
